...
import { APP_CONFIG } from "@/core/configs";
import { useAlert } from "@/features/_global";
import { KeyRound, Mail, Loader2, ArrowRight } from "lucide-react"; // Tambahkan Loader2
import { FormEventHandler, useState } from "react";
      ...
      <div className="space-y-8">
        ...
        {!isSent ? (
          <form onSubmit={submit} className="space-y-6">
            <div className="space-y-3">
              <Label htmlFor="email" className="text-slate-300 text-[13px] ml-1">
                Alamat Email
              </Label>
              <div className="relative group">
                ...
                <Input
                  ...
                  value={email}
                  onChange={({ target: { value } }) => setEmail(value)}
                  className="h-12 pl-12 bg-white/[0.03] border-white/10 text-white placeholder:text-white/20 focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/5 transition-all rounded-2xl"
                />
              </div>
            </div>
          ...
          <div className="p-6 bg-blue-500/5 border border-blue-500/10 rounded-3xl text-center space-y-4 animate-in fade-in zoom-in duration-500">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium text-white">Email Terkirim!</p>
        ...
        <div className="text-center">
          <p className="text-[13px] text-slate-500">
            {lang.text("hasAccount")}{" "}
            <Link to="/auth/login" className="text-slate-200 hover:text-blue-400 font-medium transition-colors">
              {lang.text("login")}
            </Link>
          </p>
        </div>
